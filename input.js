...
//   readline.question('Masukkan bilangan genap: ', (input) => {
...
const prompt = require('prompt-sync')({sigint: true});
  // Meminta pengguna untuk memasukkan bilangan genap
const input = prompt('Masukkan bilangan genap:');

// Mengubah input menjadi bilangan bulat
const x = parseInt(input);

// Memeriksa apakah input adalah bilangan negatif
if (x < 0) {
  console.log('Tidak bisa input bilangan negatif');
} 
// Memeriksa apakah input adalah bilangan ganjil
else if (x % 2 !== 0) {
  console.log('Tidak bisa input bilangan ganjil');
} 
// Jika input adalah bilangan genap, menghitung akar pangkat 2
else {
  const result = Math.sqrt(x);
  console.log(`Akar pangkat 2 dari ${x} adalah ${result}`);
}
